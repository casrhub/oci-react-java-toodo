...
import { API_TAREAS, API_SUBTAREAS } from '../api/index';
...
function Home() {
  ...
  function fetchSubTareas(tareaId) {
    return fetch(`${API_SUBTAREAS}?tareaId=${tareaId}`).then((response) =>
      response.ok ? response.json() : Promise.reject('Failed to fetch subtareas')
    );
  }
  ...
  function reloadOneItem(id) {
    fetch(API_TAREAS + '/' + id)
      .then((response) =>
        response.ok ? response.json() : Promise.reject(new Error('Something went wrong ...'))
      )
      .then((result) => {
  ...
  function addItem(titulo, descripcion, usuarioId, equipoId, proyectoId, horasEstimadas) {
    ...
    fetch(API_TAREAS, {
      ...
      .then((response) =>
        response.ok ? response.json() : Promise.reject(new Error('Failed to create task'))
      )
      .then((createdTask) => {
        if (horasEstimadas > 4) {
  ...
  return (
    <div className="App">
      ...
      <Dialog open={completeDialogOpen} onClose={() => setCompleteDialogOpen(false)}>
        ...
        <DialogActions>
          <Button onClick={() => setCompleteDialogOpen(false)} color="secondary">
            Cancel
          </Button>
          <Button
            onClick={() => {
